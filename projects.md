...
#### *Approach*:
We systematically compared the effect of sensor fusion across legs (unilateral, contralateral, or bilateral) and modalities (myoelectric, kinematic, intertial, all) on offline prediction accuracy using the ENABL3S dataset. We identified relevant windows before gait events, extracted heuristic features, and used PCA for dimensionality reduction. We used k-fold cross-validation to compare different sensor configurations and applied three types of classifiers (linear discriminant, SVM, and neural network) to investigate the effect of data dimensionality on accuracy. We also performed sequential forward sensor selection to determine the type and number of contralateral sensors needed to signficantly improve accuracy. Lastly, we collected data from one amputee subject wearing an IMU sensor on his non-prosthesis side as a proof-of-concept for the generalizability and benefit of using bilateral control information in an impaired population. 
